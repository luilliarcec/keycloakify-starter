            ...
            <form id="kc-otp-login-form" className={kcClsx("kcFormClass")} action={url.loginAction} method="post">
                {otpLogin.userOtpCredentials.length > 1 && (
                    <div className={kcClsx("kcFormGroupClass")}>
                        <div className="otpCredentialsWrapperClass">
                            {otpLogin.userOtpCredentials.map((otpCredential, index) => (
                                <Fragment key={index}>
                                    <label htmlFor={`kc-otp-credential-${index}`} className="otpCredentialItem">
                                        <input
                                            id={`kc-otp-credential-${index}`}
                                            className="otpCredentialInputCheck"
                                            type="radio"
                                            name="selectedCredentialId"
                                            value={otpCredential.id}
                                            defaultChecked={otpCredential.id === otpLogin.selectedCredentialId}
                                        />
                                        <span className="otpInputLabel" tabIndex={index}>
